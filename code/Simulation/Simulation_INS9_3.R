...
## This file is generated because of Grace's comments that unweighted estimator does not have better efficiency than the validation only data
...
INS_int <- function(i, nsample, nvalidation, omega_j){
  ...
  if (omega_j==0.5) {
    NR <- nleqslv(intial4, GEE_UI_IV, jacobian=T, control=list(maxit=2000),
                  data.validation = data.validation, data.mismeasure = data.mismeasure, 
                  gamma1 = 1, gamma=c(0,gamma2), alpha1= alpha1, alpha0= alpha0, sigma_e = sigma_e, Weight =  c(omegas,rep(0,4)))
    ...
    if (!any(is.na(betahat))) {
      cov <- GEE_covIV (betahat, data.validation, data.mismeasure,
                        ...
                        fixgamma1=1, fixgamma=c(1,0), fixsigma_e=0, fixalpha1=0, fixalpha0=0, Weight = c(omegas,rep(0,4)))
      sd <- sqrt(diag(cov$covmatrix))} else {
  ...
  if (omega_j==0.5) {
    measonly <- nleqslv(intial4,GEE_UI_IV,jacobian=T, control=list(maxit=2000),
                        data.validation = data.validation, data.mismeasure = data.mismeasure, 
                        gamma1 = 1, gamma=c(0,gamma2), alpha1= -Inf, alpha0= -Inf, sigma_e = sigma_e, Weight =  c(omegas,rep(0,2)))
    
    betahat_measonly <- ifelse(abs(measonly$x)<10,measonly$x,NA)
    
    if (!any(is.na(betahat_measonly))) {
      cov <- GEE_covIV(betahat_measonly,  data.validation, data.mismeasure,
                       gamma1 = 1, gamma = c(0,gamma), alpha1= -Inf, alpha0= -Inf, sigma_e = sigma_e,
                       fixgamma1=1, fixgamma=c(1,0), fixsigma_e=0, fixalpha1=1, fixalpha0=1, Weight =  c(omegas,rep(0,2)))
      sd_measonly  <- sqrt(diag(cov$covmatrix))} else {
  ...
  if (omega_j==0.5) {
    misconly <- nleqslv(intial4,GEE_UI_IV,jacobian=T, control=list(maxit=2000),
                      data.validation = data.validation, data.mismeasure = data.mismeasure,
                      gamma1 = 1, gamma=c(0,0), alpha1= alpha1, alpha0= alpha0, sigma_e = 0, Weight =  c(omegas,rep(0,2)))
  ...
  if (!any(is.na(betahat_misconly))) {
    cov <- GEE_covIV(betahat_measonly,  data.validation, data.mismeasure,
                     ...
                     fixgamma1=1, fixgamma=c(1,1), fixsigma_e=1, fixalpha1=0, fixalpha0=0, Weight =  c(omegas,rep(0,2)))
    sd_misconly <- sqrt(diag(cov$covmatrix))} else {
...
ProjectName <- paste0("INS9_3")

cl = makeCluster(ncores, type = "PSOCK",outfile=paste0(ProjectName,".txt"))
clusterExport(cl=cl, varlist=c("seed_i", "nsample","Weightestimate","Cov01matrix","GEE_UI",
                               "GEE_SIGMA","GEE_GAMMA","GEE_GAMMA.inv","GEE_cov",
                               "GEE_UI_IV","GEE_GAMMA_IV0","GEE_GAMMA_IVI","GEE_SIGMA_IV0","GEE_SIGMA_IVI",
                               "GEE_covIV"))
registerDoParallel(cl)


# results_3 <- lapplypar(c(0,1,0.5,-1), FUN= function(x){
#   results_x <- lapplypar(1:1000, FUN = INS_int,
#                       nsample = 5000, nvalidation = 25, omega_j = x)
#   return(results_x)
# })



# results_3 <- foreach(var2=c(0,1,0.4,-1)) %:%  foreach(var1=1:1000) %dopar% {
#   start <- proc.time()
#   results_x <- INS_int(var1, nsample = 25000, nvalidation = 500, omega_j = var2)
#   end <- proc.time() - start
...
# save(results_3,file="WINV_expl_R3_1to50.RData")
load(file="WINV_expl_R3_1to50.RData")
...
for (k in 1:4) {
  ...
  Results0 <- data.frame(omega = omega[k], 
    naivebias=round(naivebias,3),naive_esd=round(naive_esd,3),naivesd=round(naivesd,3),naiveCI=percent(round(naiveCI,3)),naiveARE=(round(naive_esd/naive_esd,3)),
    biasprop=round(bias1,3),propose_esd=round(sd_emp,3),sdpropose=round(sd_mod,3),CI_propose=percent(round(CIrate,3)),ARE_propose=(round((naive_esd/sd_mod)^2,4)))
...
save(Results,file="WINV_RTable_expl_31.RData")
library(xtable)

Results


reference1 <- rep(Results[Results$omega==-1,"ARE"],each = 5)

Results$AARE <- percent(round(Results$ARE/reference1,4))
xtable(Results,digits = 3)

### Replace the theta(0.5) by the original estimator from Section 4.2

# load("WINV_RTable_expl_31.RData")
# Results2 <- Results
# load("WINV_RTable_expl_2small.RData")
# ResultsIns <- Results[1:6,c("biasprop", "propose_esd", "sdpropose", "CI_propose")]
# colnames(ResultsIns) <- c("bias","esd","MSD","CR")
# Results2$CR <- as.character(Results2$CR)
# ResultsIns$CR <- as.character(ResultsIns$CR)
# Results2[Results2$omega==0.5,c("bias","esd","MSD","CR")] <- ResultsIns
# 
# # reference1 <- Results2[rep(1:6*5,each=5),5]
# Results2$AARE <- percent(round(reference1/Results2$MSD,4))
# xtable(Results2,digits = 3)


### 4.3 Simulation 1: under different level of interation - Small Sample Size ####
results_3 <- foreach(var2=c(0,1,0.5,-1)) %:%  foreach(var1=1:1000) %dopar% {
  start <- proc.time()
  results_x <- INS_int(var1, nsample = 2000, nvalidation = 500, omega_j = var2)
  end <- proc.time() - start
  cat(paste0(var1, ": ",end[3],"\n"))
  return(results_x)
}
...
#                       nsample = 2000, nvalidation = 500, omega_j = x)
#   return(results_x)
# })
...
save(results_3,file="WINV_expl_R3_1to4.RData")
...
for (k in 1:4) {
  ...
  Results0 <- data.frame(omega = omega[k], 
                         naivebias=round(naivebias,3),naive_esd=round(naive_esd,3),naivesd=round(naivesd,3),naiveCI=percent(round(naiveCI,3)),naiveARE=(round(naive_esd/naive_esd,4)),
                         biasprop=round(bias1,3),propose_esd=round(sd_emp,3),sdpropose=round(sd_mod,3),CI_propose=percent(round(CIrate,3)),ARE_propose=(round((naive_esd/sd_mod)^2,4)))
...
save(Results,file="WINV_RTable_expl_32.RData")
library(xtable)
xtable(Results,digits = 3)
xtable(Results,digits = 4)


reference2 <- Results[rep(1:6*5,each=5),7]

Results$AARE <- percent(round(Results$ARE/reference2,4))
xtable(Results,digits = 3)


load("WINV_RTable_expl_32.RData")
Results2 <- Results
load("WINV_RTable_expl_2large.RData")
ResultsIns <- Results[1:6,c("biasprop", "propose_esd", "sdpropose", "CI_propose")]
colnames(ResultsIns) <- c("bias","esd","MSD","CR")
Results2$CR <- as.character(Results2$CR)
ResultsIns$CR <- as.character(ResultsIns$CR)
