####
# This study implements the EGMM method 
...
GEE_UI_IV_Weights <- function(theta,  data.validation, data.mismeasure,
...
SecondStep <- function(betahatEGMM1, data.validation, data.mismeasure,
  ...
  betahatEGMM2 <- optim(par = betahatEGMM1, fn = GEE_UI_IV_EGMM2, 
                        ...
                        gamma1=1, gamma=c(0,gamma2), alpha1=alpha1, alpha0=alpha0, sigma_e=sigma_e, Weight = solve(Cov01matrices$Wn[-(9:12),-(9:12)], tol = 1e-250))
  
  
  CovMat <- solve( t(Cov01matrices$GammaGMM) %*% solve(Cov01matrices$Wn, tol = 1e-250) %*% Cov01matrices$GammaGMM )
  
  
  return(list(betahat=betahatEGMM2, CovMat = CovMat))
...
results_2 <- lapply(c(1/3,1/2,2/3), FUN= function(x){
  results_x <- lapply(1:1000, FUN = INS_int,
                      nsample = 1500, nvalidation = round(x*1500) , omega_j = -1)
  return(results_x)
})
...
save(results_2,file="WINV_R2_opt.RData")
...
save(Results,file="WINV_RTable2_opt.RData")
