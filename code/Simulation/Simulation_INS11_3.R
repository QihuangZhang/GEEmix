...
clusterExport(cl=cl, varlist=c("seed_i", "nsample","Weightestimate","Cov01matrix","GEE_UI",
                               ...
                               "GEE_covIV","Cov01matrix","Cov01matrix2","SecondStep","Weightestimate"))
registerDoParallel(cl)
# re1 <- INS_int(1,nsample=2000, nvalidation=500)
# 
# 
# re1

# results_3 <- foreach(var2=c(0)) %:%  foreach(var1=1:1000) %dopar% {
#   start <- proc.time()
#   results_x <- INS_int(var1, nsample = 2000, nvalidation = 500)
#   end <- proc.time() - start
...
# save(results_3,file="WINV_R4_1_ext.RData")
load("WINV_R4_1_ext.RData")
...
for (k in 1) {
  ...
  Results0 <- data.frame( 
    ...
    biasprop=round(bias1,3),propose_esd=round(sd_emp,3),sdpropose=round(sd_mod,3),CI_propose=percent(round(CIrate,3),accuracy = .1),ARE_propose=(round((naive_esd/sd_mod)^2,4)))
...
reference1 <- c(0.026,0.012, 0.023, 0.137,0.118, 0.142)
# save(Results2,file="WINV_RTable4_ext.RData")
# load("WINV_RTable4_ext.RData")
library(xtable)




Results2$AARE <- percent(unlist(round(reference1^2/Results2$sdpropose^2,4)))
xtable(Results2,digits = 3)

### Replace the theta(0.5) by the original estimator from Section 4.2

# load("P:/Work/waterloo/2018/GEEmix/WINV_RTable31_ext.RData")
# Results2 <- Results
# load("P:/Work/waterloo/2018/GEEmix/WINV_RTable2small_ext.RData")
...
# results_3 <- foreach(var2=c(0)) %:%  foreach(var1=1:1000) %dopar% {
#   start <- proc.time()
#   results_x <- INS_int(var1, nsample = 25000, nvalidation = 500)
#   end <- proc.time() - start
#   cat(paste0(var1, ": ",end[3],"\n"))
#   return(results_x)
# }
...
# results_3 <- lapply(c(0), FUN= function(x){
#   results_x <- lapply(1:1000, FUN = INS_int,
#                       nsample = 25000, nvalidation = 500)
#   return(results_x)
# })
...
# save(results_3,file="WINV_R4_2_ext.RData")
load("WINV_R4_2_ext.RData")
...
for (k in 1) {
  ...
  sd_emp <- apply(na.omit(betas),MARGIN = 2, FUN = function(x){  
    ...
    return( sd(x.noout,na.rm = T ))   })
  sd_mod <- apply(na.omit(sds),MARGIN = 2, FUN = function(x){    
    x.noout <- x # remove_outliers(x)   
    return( mean(x.noout,na.rm = T ))   })
  CIrate <- colMeans(na.omit(CIs),na.rm = T)
  
  Results0 <- data.frame(naivebias=round(naivebias,3),naive_esd=round(naive_esd,3),naivesd=round(naivesd,3),naiveCI=percent(round(naiveCI,3)),naiveARE=(round(naive_esd/naive_esd,4)),
                         biasprop=round(bias1,3),propose_esd=round(sd_emp,3),sdpropose=round(sd_mod,3),CI_propose=percent(round(CIrate,3),accuracy = .1),ARE_propose=(round((naive_esd/sd_mod)^2,4)))
...
# xtable(Results)
# 
# save(Results,file="WINV_RTable42_ext.RData")

# load("WINV_RTable32_ext.RData")

reference2 <-  c(0.015,0.006,0.007,0.128,0.113,0.124)


Results$AARE <- percent(unlist(round(reference2^2/Results$sdpropose^2,4)))
xtable(Results,digits = 3)


# load("WINV_RTable42_ext.RData")
# # save(Results2,file="WINV_RTable42_ext.RData")
# library(xtable)
...
# load("P:/Work/waterloo/2018/GEEmix/WINV_RTable32_ext.RData")
# Results2 <- Results
# load("P:/Work/waterloo/2018/GEEmix/WINV_RTable2large_ext.RData")
# ResultsIns <- Results[1:6,c("biasprop", "propose_esd", "sdpropose", "CI_propose")]
# colnames(ResultsIns) <- c("bias","esd","MSD","CR")
# Results2$CR <- as.character(Results2$CR)
# ResultsIns$CR <- as.character(ResultsIns$CR)
