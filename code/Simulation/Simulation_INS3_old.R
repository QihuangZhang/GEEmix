...
# 2.3.3 Naive Model of only consider the measurement error ###
 ...
 if (!any(is.na(betahat_measonly))) {
    cov <- GEE_covIV(betahat_measonly,  data.validation, data.mismeasure,
      gamma1, gamma = c(0,gamma), alpha1= -Inf, alpha0= -Inf, sigma_e = sigma_e,
...
return(list(seed = seed_i[i],
 ...
 results_1 <- lapply(c(0.1,0.5,0.7), FUN= function(x){
    ...
    sigma_e = x, alpha = -2.197, gamma = 0.8)
     return(results_x)
  })

  # re1 <- INS_int(1,0.1,-1.386294, gamma = 0.8)
  ...
  save(results_1,file="INS3_R1.RData")

 Results <- NULL

for (k in 1:3) {
  results <- results_1[[k]]
  truebeta <- c(0.7,1.5,-1,0.7,-1.5,1,1,0,0.8,sigma_e_range[k],-2.197,-2.197)
  naive1coef <- NULL
  naive1sd <- NULL
  naive2coef <- NULL
  naive2sd <- NULL
 ...
 save(results_2,file="INS3_R2.RData")
 ...
 for (k in 1:3) {
    results <- results_2[[k]]
    truebeta <- c(0.7,1.5,-1,0.7,-1.5,1,1,0,0.8,0.1,alpha_range[k],alpha_range[k])
    naive1coef <- NULL
    naive1sd <- NULL
    naive2coef <- NULL
    naive2sd <- NULL
 ...
 for (i in 1:1000){
  ...
  sd_mod_misconly <- apply(na.omit(sds),MARGIN = 2, FUN = function(x){
        x.noout <-  remove_outliers(x)
        return( mean(x.noout,na.rm = T ))
      })
  CI_misconly <- colMeans(na.omit(CImisconly),na.rm = T)
...
results_3 <- lapply(c(0,-0.8,0.8), FUN= function(x){
  results_x <- lapply(1:1000, FUN = INS_int,
      sigma_e = 0.1, alpha = -2.197, gamma = x)
  return(results_x)
  })
 ...
 save(results_3,file="INS3_R3.RData")
 ...
 Results <- NULL
  ...
  for (k in 1:3) {
    results <- results_3[[k]]
    truebeta <- c(0.7,1.5,-1,0.7,-1.5,1,1,0,gamma_range[k],0.1,-2.197,-2.197)
    naive1coef <- NULL
    naive1sd <- NULL
    naive2coef <- NULL
    naive2sd <- NULL
