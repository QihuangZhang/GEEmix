...
data_GWA_Pre <- data.frame(id = phenotype$id,Y1 = phenotype$tibia,Y2 = phenotype$abnormalbone, discard = ifelse(phenotype$discard=="no",1,0),
                           BW = phenotype$bw0, BMD=phenotype$BMD, Y1star=phenotype$TAstar, Y2star=phenotype$BCstar)
...
Covariates<-cbind(Intercept=rep(1,length(genotype[,2])),X905=rs27338905,data_GWAS[,c("BW","BMD")],
...
CovMis2 <- as.matrix(data.frame(intercept=rep(1,nsample),BMD=Covariates_all$BMD))
