...
# GEE_UI_safe <- function(theta, Y1star, Y2star, DesignMatrix1, DesignMatrix2, CovMis1, CovMis2, alpha1, alpha0){
#   cat(theta, " \n")
# return(GEE_UfuncIns_safe(Y1star, Y2star, DesignMatrix1, DesignMatrix2, CovMis1, CovMis2,
#                          theta[1:4], theta[5:8], sigma = theta[9], xi = theta[10], 
#                       gamma1 = 1, gamma=c(0,0), alpha1, alpha0, sigma_e = 0.77))
# }
...
mismeasuredata <- data.frame(Y1star=Y1star[1:nsample], Y2star=Y2star[1:nsample],
                             Covariates1[1:nsample,], CovMis1[1:nsample,], 
                             CovMis2[1:nsample,], Y1=Y1[1:nsample], Y2=Y2[1:nsample])
...
intial2 <- c(naive.model1$coefficients,naive.model2$coefficients,0.01,0)
cat("Initial value:", intial2, "\n")

## 2.2.2 True model ####
true.model1 <- lm(Y1 ~ X109 + PC1 + PC2, data = mismeasuredata)
true.model2 <- glm(Y2 ~ X109 + PC1 + PC2, family = binomial(link = logit), data = mismeasuredata)

## 2.2.3 the proposed approach ####
## Mainly interested parameters
NR <- nleqslv(intial2, GEE_UI, jacobian=T, control=list(maxit=2000),
              Y1star = mismeasuredata$Y1star,
              Y2star = mismeasuredata$Y2star, 
              DesignMatrix1 = as.matrix(mismeasuredata[,3:6]),
              DesignMatrix2 = as.matrix(mismeasuredata[,3:6]),
              CovMis1 = as.matrix(mismeasuredata[,7:8]),
              CovMis2 = as.matrix(mismeasuredata[,9]),
              alpha1= -2.95, alpha0= -2.95, sigma_e = 0.1)
...
truebeta <- c(true.model1$coefficients,true.model2$coefficients)
sdtrue <- sqrt(c(diag(vcov(true.model1)),diag(vcov(true.model1))))

Zvalue_true <- truebeta/sdtrue
pvalue_true <- 2*(1-pnorm(abs(Zvalue_true)))
...
Table_numeric <- data.frame(naivebeta=c(naivebeta,rep(0,6)),
                            ...
                            truebeta=c(truebeta,rep(0,6)),
                            sdtrue=c(sdtrue,rep(0,6)),
                            Z_true=c(Zvalue_true,rep(0,6)),
                            p_true=c(pvalue_true,rep(0,6)),
                            propobeta=c(betahat,rep(0,4)),
                            proposd = c(sd,rep(0,4)),
                            propoZ = c(Zvalue,rep(0,4)),
                            propop = c(pvalue,rep(0,4)))
