...
# library(ggplot2)
# 
# pdf(file = "output/Screeplot.pdf",height = 5, width = 9)
# 
# Scree$Selection <- c(rep("Selected",2),rep("Not Selected",8))
# 
# ggplot(Scree)  +
#   geom_bar(aes(x=rownamesa, y=variance, fill=Selection),stat="identity")+
#   geom_line(aes(x=rownamesa, y=percentage*max(Scree$variance)*10, group = 1),stat="identity", size = 1.2, color= "#CD7E59")+
#   labs(x = "Principal Components", y = "Variance") +
#   scale_fill_manual(values = c("#5A6868","#DDB247"))  +
...
load("N://Work/waterloo/2018/mixResponse/file/genotrunc.RData")
phenotype<-read.csv(file="N://Work/waterloo/2018/mixResponse/data/pheno.csv")
load("N://Work/waterloo/2018/GEEmix/file/loading.RData")
load("N://Work/waterloo/2018/GEEmix/file/rs27338905.RData")
...
GEE_UI_IV_EGMM1 <- function(theta,  data.validation, data.mismeasure,
  ...
  UI_Main <- GEE_UfuncIns(Y1star=data.mismeasure$Y1star,
                          Y2star=data.mismeasure$Y2star,
                          DesignMatrix1 = as.matrix(data.mismeasure[,3:6]), 
                          DesignMatrix2 = as.matrix(data.mismeasure[,3:6]), 
                          CovMis1 = as.matrix(data.mismeasure[,7:8]),
...
GEE_UI_IV_EGMM2 <- function(theta,  data.validation, data.mismeasure, Weight){
  ...
  UI_Validation <- GEE_UfuncInsIVWeight(Y1star=data.mismeasure$Y1star,
                                        ...
                                        Weight = rep(0,10),
                                        beta1=theta[1:4], beta2=theta[5:8], sigma = theta[9], xi = theta[10],  
                                        gamma1=theta[11], gamma=c(theta[12],0,theta[13]), alpha1=theta[15], alpha0=theta[16], sigma_e=theta[14])
...
SecondStep <- function(theta, data.validation, data.mismeasure,
                       gamma1, gamma, alpha1, alpha0, sigma_e,
                       fixgamma1, fixgamma, fixsigma_e, fixalpha1, fixalpha0){
  nomegas <- 10
  nsco <- length(c(fixgamma1,fixgamma,fixsigma_e,fixalpha1,fixalpha0))-sum(c(fixgamma1,fixgamma,fixsigma_e,fixalpha1,fixalpha0))
  ...
  if (!any(is.na(theta))) {
    cov1 <- GEE_covIV (theta[1:10], data.validation, data.mismeasure, Weight = c(rep(1,nomegas),rep(0,nsco)),
                       gamma1=theta[11], gamma=c(theta[12],0,theta[13]), alpha1=theta[15], alpha0=theta[16], sigma_e=theta[14],
                       fixgamma1=fixgamma1, fixgamma=fixgamma, fixsigma_e=fixsigma_e, fixalpha1=fixalpha1, fixalpha0=fixalpha0)}
  ...
  ### variance estimation with validation data
  if (!any(is.na(theta))) {
    cov0 <- GEE_covIV (theta[1:10], data.validation, data.mismeasure, Weight = c(rep(0,nomegas),rep(0,nsco)),
                       gamma1=theta[11], gamma=c(theta[12],0,theta[13]), alpha1=theta[15], alpha0=theta[16], sigma_e=theta[14],
                       fixgamma1=fixgamma1, fixgamma=fixgamma, fixsigma_e=fixsigma_e, fixalpha1=fixalpha1, fixalpha0=fixalpha0)}
  
  Cov01matrices <- Cov01matrix2(cov1,cov0,nomegas,nsco)
  
  betahatEGMM2 <- optim(par = theta, fn = GEE_UI_IV_EGMM2, 
                        method = "L-BFGS-B", data.validation = data.validation, data.mismeasure = data.mismeasure,  Weight = solve(Cov01matrices$Wn[-(11:16),-(11:16)], tol = 1e-250),
                        lower = theta-0.2*abs(theta), upper = theta+0.2*abs(theta))
  
  
  CovMat <- solve( t(Cov01matrices$GammaGMM) %*% solve(Cov01matrices$Wn, tol = 1e-250) %*% Cov01matrices$GammaGMM , tol = 1e-250)
...
#### 2. Analysis  without considering optimal weights ####

intial2 <- c(betahat[1:8],0.05,0)
nomegas <- length(intial2)

NR1 <- nleqslv(intial2, GEE_UI_IV, jacobian=T, control=list(maxit=2000),
               data.validation = validationdata, data.mismeasure = mismeasuredata, 
               Weight = c(rep(0.5,nomegas),rep(0.5,6)),
               gamma1 = gamma1, gamma = c(gamma0,0,gamma3), alpha1= alpha1, alpha0= alpha0, sigma_e = sigma_e)

betahat2 <- ifelse(abs(NR1$x)<100,NR1$x,NA)

### variance estimation with validation data
if (!any(is.na(betahat))) {
  cov2 <- GEE_covIV (betahat2, data.validation = validationdata, data.mismeasure = mismeasuredata, Weight = c(rep(0.5,nomegas),rep(0.5,6)),
                     gamma1 = gamma1, gamma = c(gamma0,0,gamma3), alpha1 = alpha1, alpha0 = alpha0, sigma_e = sigma_e,
                     fixgamma1=0, fixgamma=c(0,1,0), fixsigma_e=0, fixalpha1=0, fixalpha0=0)
  sd2 <- sqrt(diag(cov2$covmatrix))
  }



betahat2 <- c(betahat2,gamma1,gamma0,gamma3,sigma_e,alpha1,alpha0)

Zvalue2 <- betahat2/sd2
pvalue2 <- 2*(1-pnorm(abs(Zvalue2)))

naivebeta2 <- c(naive.model1$coefficients,naive.model2$coefficients)
sdnaive2 <- sqrt(c(diag(vcov(naive.model1)),diag(vcov(naive.model1))))

Zvalue_naive2 <- naivebeta2/sdnaive2
pvalue_naive2 <- 2*(1-pnorm(abs(Zvalue_naive2)))

Table_numeric2 <- data.frame(naivebeta=c(naivebeta2,rep(0,8)),
                            sdnaive=c(sdnaive2,rep(0,8)),
                            Z_naive=c(Zvalue_naive2,rep(0,8)),
                            p_naive=c(pvalue_naive2,rep(0,8)),
                            propobeta=betahat2,
                            proposd = sd2,
                            propoZ = Zvalue2,
                            propop = pvalue2)
...
library(xtable)
xtable(Table_numeric2,digits = 3)
...
# intial3 <- c(naive.model1$coefficients,naive.model2$coefficients,10,0)

intial3 <- c(betahat[1:8],6.5,0)
# nomegas <- length(intial3)

## The first step

# betahatEGMM1 <- optim(par = intial3, fn = GEE_UI_IV_EGMM2, 
#                       method = "BFGS", data.validation = validationdata, data.mismeasure = mismeasuredata,
#                       gamma1=gamma1, gamma=c(gamma0,gamma2,gamma3), alpha1=alpha1, alpha0=alpha0, sigma_e=sigma_e,
#                       Weight = diag(c(rep(1,8),0.0001,0.0001,rep(1e-12,8),1e-15,1e-15)))


# GEE_UI_IV_EGMM2(intial3,  data.validation = validationdata, data.mismeasure = mismeasuredata,
#                 Weight = c(rep(1,8),0.0001,0.0001,rep(1e-12,8),1e-15,1e-15), 
#                 gamma1=gamma1, gamma=c(gamma0,gamma2,gamma3), alpha1=alpha1, alpha0=alpha0, sigma_e=sigma_e)

## The second step

EGMM2  <- SecondStep(betahat2, validationdata, mismeasuredata,
                     fixgamma1=0, fixgamma=c(0,1,0), fixsigma_e=0, fixalpha1=0, fixalpha0=0)
# return(EGMM2)

betahat3 <- EGMM2$betahat

### variance estimation with validation data
if (!any(is.na(betahat3))) {
  cov3 <- EGMM2$CovMat
  sd3 <- sqrt(diag(cov3))} else {
    sd3 <- rep(NA,length(betahat))
  }


betahat3 <- c(betahat3)

Zvalue3 <- betahat3/sd3
pvalue3 <- 2*(1-pnorm(abs(Zvalue3)))

# naivebeta3 <- c(naive.model1$coefficients,naive.model2$coefficients)
# sdnaive3 <- sqrt(c(diag(vcov(naive.model1)),diag(vcov(naive.model1))))

# Zvalue_naive2 <- naivebeta2/sdnaive2
# pvalue_naive2 <- 2*(1-pnorm(abs(Zvalue_naive2)))

Table_numeric3 <- data.frame(propobeta=betahat3,
                             proposd = sd3,
                             propoZ = Zvalue3,
                             propop = pvalue3)
...
library(xtable)
xtable(Table_numeric3,digits = 3)
