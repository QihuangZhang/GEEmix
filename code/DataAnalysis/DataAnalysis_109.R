...
## 0.1 Set file path ####
setwd("P:/Work/waterloo/2018/GEEmix")
...
load("P:/Work/waterloo/2018/mixResponse/file/genotrunc.RData")
phenotype<-read.csv(file="P:/Work/waterloo/2018/mixResponse/data/pheno.csv")
load("P:/Work/waterloo/2018/GEEmix/file/loading.RData")
...
NR <- nleqslv(intial2, GEE_UI, Y1star=Y1star, Y2star=Y2star, DesignMatrix1=Covariates1, DesignMatrix2=Covariates2,
              CovMis1=CovMis1, CovMis2=CovMis2, alpha1= alpha1, alpha0= alpha0, jacobian=T, control=list(maxit=2000), sigma_e=sigma_e)


for (sigma_e in c(0.72,0.77,0.82)){
  Results109 <- c("(Intercept1)","X","PC1","PC2","(Intercept2)","X","PC1","PC2","sigma_e","xi")
  for (alphavalue in c(-2.5,-2,-1.5)){
    NR <- nleqslv(intial2, GEE_UI, Y1star=Y1star, Y2star=Y2star, DesignMatrix1=Covariates1, DesignMatrix2=Covariates2,
                  CovMis1=CovMis1, CovMis2=CovMis2, alpha1=alphavalue, alpha0=alphavalue, jacobian=T, control=list(maxit=2000), sigma_e=0.77)
    ...
    if (!any(is.na(betahat))) {
      cov <- GEE_cov(betahat, Y1star, Y2star, DesignMatrix1=Covariates1, DesignMatrix2=Covariates2, 
                     ...
                     gamma=c(0,0), alpha1=alphavalue, alpha0=alphavalue, sigma_e=0.77)
      sd <- sqrt(diag(cov))
    }
