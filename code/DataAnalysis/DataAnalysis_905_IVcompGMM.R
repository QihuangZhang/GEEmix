...
# library(ggplot2)
# 
# Scree$Selection <- rep(c("Selected","Not Selected"),each = 5)
# 
# pdf(file = "output/Screeplot.pdf",height = 7, width = 10)
# 
# ggplot(Scree)  +
...
#   scale_fill_manual(values = c("#566573","#CD5C5C"))  +
#   theme_minimal() +
...
GEE_UI_IV_EGMM2 <- function(theta,  data.validation, data.mismeasure, Weight){
  ...
  UI_Validation <- GEE_UfuncInsIVWeight(Y1star=data.mismeasure$Y1star,
                                        ...
                                        Weight = rep(0,10),
                                        beta1=theta[1:4], beta2=theta[5:8], sigma = theta[9], xi = theta[10], 
                                        gamma1=theta[11], gamma=theta[12:14], alpha1=theta[16:17], alpha0=theta[18:19], sigma_e=theta[17])
...
SecondStep <- function(theta, data.validation, data.mismeasure,
  ...
  betahatEGMM2 <- optim(par = theta, fn = GEE_UI_IV_EGMM2, 
                        method = "L-BFGS-B", data.validation = data.validation, data.mismeasure = data.mismeasure, Weight = solve(Cov01matrices$Wn[-(11:19),-(11:19)], tol = 1e-250),
...
intial2 <- c(naive.model1$coefficients,naive.model2$coefficients,10,0)

intial2 <- c(betahat[1:8],9.21,0)
nomegas <- length(intial2)

NR1 <- nleqslv(intial2, GEE_UI_IV, jacobian=T, control=list(maxit=2000, allowSingular=TRUE),
               data.validation = validationdata, data.mismeasure = mismeasuredata, 
               Weight = c(rep(0.5,nomegas),rep(0.5,9)),
               gamma1 = gamma1, gamma = c(gamma0,gamma2,gamma3), alpha1= alpha1, alpha0= alpha0, sigma_e = sigma_e)

betahat2 <- ifelse(abs(NR1$x)<1000,NR1$x,NA)

### variance estimation with validation data
if (!any(is.na(betahat))) {
  cov2 <- GEE_covIV (betahat2, data.validation = validationdata, data.mismeasure = mismeasuredata, Weight = c(rep(0.5,nomegas),rep(0.5,9)),
                     gamma1 = gamma1, gamma = c(gamma0,gamma2,gamma3), alpha1 = alpha1, alpha0 = alpha0, sigma_e = sigma_e,
                     fixgamma1=0, fixgamma=c(0,0,0), fixsigma_e=0, fixalpha1=c(0,0), fixalpha0=c(0,0))
...
betahat2 <- c(betahat2,gamma1,gamma0,gamma2,gamma3,sigma_e,alpha1,alpha0)
...
Table_numeric2 <- data.frame(propobeta=betahat2,
                             proposd = sd2,
                             propoZ = Zvalue2,
                             propop = pvalue2)
...
# betahat3 <- c(betahat3,gamma1,gamma0,gamma2,gamma3,sigma_e,alpha1,alpha0)
